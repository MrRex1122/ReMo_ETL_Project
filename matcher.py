...
class ReMoMatcher:
    ...
    def _infer_other_subfamily_for_item(self, item: Dict[str, Any]) -> str:
        ...
        if any(
            ...
            any(token in search_text for token in ("щит распредел", "щиток", "электрощит", "корпус распредел", "корпус учетно", "щрв", "щрн", "щурв", "щурн"))
            and any(token in search_text for token in ("встраив", "навес", "модул", "распредел", "учет", "щит"))
