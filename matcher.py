...
class ReMoMatcher:
    ...
    def _infer_other_subfamily_for_item(self, item: Dict[str, Any]) -> str:
        ...
        if any(token in branch_path for token in ("затворы поворотные дисковые", "краны шаровые стальные", "краны шаровые латунные для воды", "краны шаровые пнд", "клапаны электромагнитные (соленоидные)", "клапаны регулирующие чугунные", "клапаны запорные (вентили) стальные", "клапаны обратные чугунные", "задвижки чугунные клиновые")) or (
            ...
            or ("клапан" in search_text and any(token in search_text for token in ("регулирующ", "запорн", "вентил", "обратн")))
            or "задвижк" in search_text
        ):
            return "industrial_valve"
        if "промышленные вертикальные центробежные насосы" in branch_path or (
            any(token in search_text for token in ("насос", "pump", "центробежн", "вертикальн"))
        ...
        if "резьбомеры" in branch_path or (
            ...
            return "thread_gauge"
        if "наборы резьбонарезного инструмента" in branch_path or (
            any(token in search_text for token in ("набор резьбонарезного инструмента", "наборы резьбонарезного инструмента", "tap and die set", "threading tool set"))
            and not any(token in search_text for token in ("резьбомер", "калибр", "сверл", "коронк", "фрез"))
        ):
            return "threading_tool_set"
        if "напильники" in branch_path or (
            any(token in search_text for token in ("напильник", "напильники", "надфиль", "надфили", "hand file", "metal file"))
        ...
        if "фрезы и наборы фрез для ручных фрезеров" in branch_path or (
            (
                any(token in search_text for token in ("фрезы и наборы фрез для ручных фрезеров", "фреза для ручного фрезера", "набор фрез для ручного фрезера", "router bit", "router cutter"))
                or ("фрез" in search_text and "фрезер" in search_text and any(token in search_text for token in ("ручн", "router")))
            )
        ...
        if any(token in branch_path for token in ("костюмы летние", "костюмы утепленные", "костюмы сварщика", "брюки полукомбинезоны", "куртки утепленные", "сигнальные костюмы", "куртки летние", "жилеты утепленные", "халаты рабочие")) or (
            ...
            return "workwear"
        if any(token in branch_path for token in ("ботинки рабочие", "полуботинки рабочие", "ботинки утепленные", "сапоги резиновые")) or (
            any(token in search_text for token in ("ботинки рабочие", "полуботинки рабочие", "ботинки утепленные", "сапоги резиновые", "защитная обувь", "рабочие сапоги", "защитные сапоги", "work boots", "safety shoes"))
        ...
        if "защитные очки" in branch_path or (
            any(token in search_text for token in ("защитные очки", "очки защитные", "safety glasses", "protective eyewear"))
            and not any(token in search_text for token in ("очки виртуальной реальности", "солнцезащитные", "маска сварщика", "щиток лицевой"))
        ):
            return "safety_glasses"
        if "каски" in branch_path or (
            any(token in search_text for token in ("каска", "каски", "защитная каска", "строительная каска", "safety helmet", "hard hat"))
            and not any(token in search_text for token in ("маска", "щиток", "очк", "cap", "бейсболка"))
        ):
            return "protective_helmet"
        if "ручные пресс-клещи и кримперы" in branch_path or (
            any(token in search_text for token in ("ручные пресс-клещи и кримперы", "пресс-клещи", "пресс клещи", "кримпер", "клещи для обжима", "обжимные клещи", "crimping tool", "crimping pliers"))
            and not any(token in search_text for token in ("токоизмер", "clamp meter", "переставн", "болторез", "длинногубцы"))
        ):
        ...
        if "накидные ключи" in branch_path or (
            any(token in search_text for token in ("накидной ключ", "накидные ключи", "ключ накидной", "ring wrench", "ring spanner", "box wrench"))
            and not any(token in search_text for token in ("комбинирован", "рожков", "имбус", "шестигран", "разводной", "трубный ключ", "ключ доступа"))
        ):
            return "ring_wrench"
        if "разводные ключи" in branch_path or (
            any(token in search_text for token in ("разводной ключ", "разводные ключи", "ключ разводной", "adjustable wrench", "crescent wrench"))
            and not any(token in search_text for token in ("комбинирован", "рожков", "накидн", "имбус", "шестигран", "трубный ключ", "ключ доступа"))
        ):
            return "adjustable_wrench"
        if "наборы отверток" in branch_path or (
            any(token in search_text for token in ("набор отверток", "наборы отверток", "screwdriver set", "set of screwdrivers"))
            and not any(token in search_text for token in ("битодержатель", "набор бит", "torx bit", "phillips bit"))
        ):
            return "screwdriver_set"
        if "биты шестигранные hex" in branch_path or (
            any(token in search_text for token in ("биты шестигранные hex", "бита шестигранная", "hex bit", "allen bit"))
            and not any(token in search_text for token in ("отвертка", "битодержатель", "torx", "phillips", "шлицев"))
        ):
            return "hex_bit"
        ...
        if "штангенциркули" in branch_path or (
            ...
            return "caliper"
        if "микрометры" in branch_path or (
            any(token in search_text for token in ("микрометр", "микрометры", "outside micrometer", "digital micrometer"))
            and not any(token in search_text for token in ("микрометрический винт", "головка микрометрическая"))
        ):
            return "micrometer"
        if "нутромеры" in branch_path or (
            any(token in search_text for token in ("нутромер", "нутромеры", "bore gauge", "inside gauge"))
            and not any(token in search_text for token in ("микрометр", "штангенциркуль", "резьбомер", "индикатор часового типа"))
        ):
            return "bore_gauge"
        if "измерительные рулетки" in branch_path or (
            any(token in search_text for token in ("измерительная рулетка", "измерительные рулетки", "рулетка", "tape measure", "measuring tape"))
            and not any(token in search_text for token in ("рулетка казино", "рулетка игровая"))
        ):
            return "tape_measure"
        if "уровни пузырьковые" in branch_path or (
            any(token in search_text for token in ("уровень пузырьковый", "уровни пузырьковые", "строительный уровень", "spirit level", "bubble level"))
            and not any(token in search_text for token in ("лазерный", "нивелир", "уровнемер"))
        ):
            return "spirit_level"
        if "валики" in branch_path or (
            any(token in search_text for token in ("валик", "валики", "малярный валик", "paint roller", "roller sleeve"))
            and not any(token in search_text for token in ("роликоподшип", "подшип", "ремень", "конвейер"))
        ):
            return "paint_roller"
        if "кисти плоские флейцевые" in branch_path or (
            any(token in search_text for token in ("кисть плоская флейцевая", "кисти плоские флейцевые", "кисть флейцевая", "флейцевая кисть", "paint brush", "flat brush"))
            and not any(token in search_text for token in ("зубная щетка", "крацовка", "корщетка"))
        ):
            return "paint_brush"
        if "корщетки" in branch_path or (
            any(token in search_text for token in ("корщетка", "корщетки", "крацовка", "щетка крацовка", "wire brush wheel", "cup brush"))
        ...
        if "хомуты для труб" in branch_path or (
            any(token in search_text for token in ("хомуты для труб", "хомут для труб", "трубный хомут", "pipe clamp", "pipe hanger"))
            and not any(token in search_text for token in ("кабельная стяжка", "хомут червячный", "хомут силовой", "strut clamp"))
        ):
            return "pipe_clamp"
        if any(token in branch_path for token in ("фитинги для наружной канализации", "фитинги для внутренней канализации", "фитинги шумопоглощающие для канализации")) or (
            (
                any(token in search_text for token in ("фитинги для наружной канализации", "фитинги для внутренней канализации", "фитинги шумопоглощающие для канализации", "канализационный фитинг", "sewer fitting", "drain fitting"))
                or (
                    any(token in search_text for token in ("отвод", "тройник", "муфта", "ревизи", "крестовин", "переход"))
                    and any(token in search_text for token in ("канализ", "наружн", "внутрен", "sewer", "drain"))
                )
            )
            and not any(token in search_text for token in ("pex", "pert", "пнд", "полипропилен", "латун", "трубный хомут", "сифон"))
        ):
            return "sewer_fitting"
        if "сифоны" in branch_path or (
            any(token in search_text for token in ("сифон", "сифоны", "siphon", "bottle trap", "trap for sink"))
            and not any(token in search_text for token in ("фитинг", "канализационная труба", "трап", "дренажный клапан"))
        ):
            return "siphon"
        if any(token in branch_path for token in ("смесители для ванны и душа", "смесители для мойки")) or (
            any(token in search_text for token in ("смеситель", "смесители для ванны и душа", "смесители для мойки", "faucet", "mixer tap"))
            and not any(token in search_text for token in ("картридж", "термостат", "душев", "лейка"))
        ):
            return "faucet"
        ...
        if "специальный инструмент для авторемонта" in branch_path or (
            any(token in search_text for token in ("специальный инструмент для авторемонта", "инструмент для авторемонта", "вакуумметр", "ареометр", "маслосборный", "противооткатный башмак", "авторемонтный инструмент"))
        ...
        if "домкраты" in branch_path or (
            any(token in search_text for token in ("домкрат", "домкраты", "hydraulic jack", "bottle jack", "floor jack"))
            and not any(token in search_text for token in ("подъемник", "стойка трансмиссионная", "автокран"))
        ):
            return "jack"
        ...
        if "абразивные отрезные диски" in branch_path or (
            any(token in search_text for token in ("абразивный отрезной диск", "абразивные отрезные диски", "абразивный отрезной круг", "круг абразивный отрезной", "отрезной диск", "отрезной круг", "cutting disc", "cut-off wheel"))
            and not any(token in search_text for token in ("алмазн", "пильный", "тормозной диск", "затвор дисковый"))
        ):
        ...
        if "бокорезы и кусачки" in branch_path or (
            any(token in search_text for token in ("бокорез", "бокорезы", "кусачки", "боковые кусачки", "cutting pliers", "diagonal cutter", "side cutter"))
            and not any(token in search_text for token in ("клещи токоизмер", "clamp meter", "переставн", "обжим", "press tool", "crimp"))
        ):
            return "cutting_pliers"
        if "длинногубцы утконосы и круглогубцы" in branch_path or (
            any(token in search_text for token in ("длинногубцы", "утконосы", "круглогубцы", "needle nose pliers", "long nose pliers", "round nose pliers"))
        ...
        if "струбцины" in branch_path or (
            ...
            return "clamp_tool"
        if "съемники ручные" in branch_path or (
            any(token in search_text for token in ("съемник", "съемники ручные", "puller tool", "gear puller", "bearing puller"))
            and not any(token in search_text for token in ("экстрактор", "домкрат", "подъемник", "магнитный съемник"))
        ):
            return "manual_puller"
        if "ножи строительные" in branch_path or (
            any(token in search_text for token in ("нож строительный", "ножи строительные", "utility knife", "snap-off knife"))
            and not any(token in search_text for token in ("кухонный", "канцелярский ножницы", "резак для труб", "резец"))
        ):
            return "utility_knife"
        if "леска для триммеров" in branch_path or (
            any(token in search_text for token in ("леска для триммеров", "леска для триммера", "триммерная леска", "trimmer line", "nylon line"))
        ...
        if any(token in branch_path for token in ("саморезы универсальные", "саморезы гипсокартон-дерево", "саморезы гипсокартон дерево", "саморезы гипсокартон-металл", "саморезы гипсокартон металл")) or (
            (
                ...
                or ("саморез" in search_text and "гипсокартон" in search_text)
            )
        ...
        if "сверла по металлу" in branch_path or (
            any(
                token in search_text
                for token in ("сверло по металлу", "сверла по металлу", "drill bit", "metal drill", "hss drill")
            )
            and not any(token in search_text for token in ("метчик", "плашк", "коронк", "зенкер", "держател", "tap", "thread tap"))
        ):
            return "drill_bit_metal"
        if "сверла по дереву" in branch_path or (
            any(token in search_text for token in ("сверло по дереву", "сверла по дереву", "wood drill bit", "brad point bit", "auger bit"))
            and not any(token in search_text for token in ("металл", "metal", "бетон", "masonry", "коронк", "бур", "sds", "tap", "thread tap"))
        ):
            return "wood_drill_bit"
        if any(token in branch_path for token in ("буры sds-plus", "буры sds-max", "сверла по бетону")) or (
            any(
                token in search_text
        ...
        if "коронки по металлу" in branch_path or "коронки" in branch_path or (
            ...
            return "hole_saw"
        if "зенкеры и зенковки" in branch_path or (
            any(token in search_text for token in ("зенкер", "зенковка", "зенкеры и зенковки", "countersink", "countersink bit"))
        ...
        if any(token in branch_path for token in ("зубила sds-plus", "зубила sds-max")) or (
            ...
            and not any(token in search_text for token in ("бур", "сверл", "коронк", "drill", "hole saw", "core bit", "металл", "metal", "дерев", "wood"))
        ):
            return "sds_chisel"
        ...
        if "радиаторы стальные панельные" in branch_path or "радиатор" in search_text or "radiator" in search_text:
            return "radiator"
        if any(token in branch_path for token in ("конвекторы внутрипольные", "конвекторы напольные")) or (("конвектор" in search_text or "convector" in search_text) and any(token in search_text for token in ("внутрипол", "напольн"))):
            return "floor_convector"
        if "нагревательные маты" in branch_path or (
            any(token in search_text for token in ("нагревательный мат", "нагревательные маты", "тёплый пол мат", "теплый пол мат", "heating mat"))
            and not any(token in search_text for token in ("кабель нагревательный", "терморегулятор", "тепловентилятор", "коврик"))
        ):
            return "heating_mat"
        if "водонагреватели электрические накопительные" in branch_path or (
            any(token in search_text for token in ("водонагреватель", "накопительный водонагреватель", "бойлер", "storage water heater", "electric water heater"))
            and not any(token in search_text for token in ("проточный", "газовый", "теплообменник", "котел"))
        ):
            return "storage_water_heater"
        if "конвекторы электрические" in branch_path or (
            any(token in search_text for token in ("конвектор электрический", "конвекторы электрические", "electric convector", "electric heater convector"))
            and not any(token in search_text for token in ("внутрипольн", "напольн", "радиатор", "тепловентилятор"))
        ):
            return "electric_convector"
        if "коллекторные группы для теплого пола" in branch_path or (
            any(token in search_text for token in ("коллекторная группа для теплого пола", "коллекторные группы для теплого пола", "floor heating manifold", "underfloor heating manifold"))
            and not any(token in search_text for token in ("смеситель", "насосн", "терморегулятор", "труба"))
        ):
        ...
        if "термоусаживаемые изделия" in branch_path or "термоусаж" in search_text or "термоусад" in search_text or "heat shrink" in search_text or "shrink tube" in search_text:
            return "heat_shrink"
        if any(token in branch_path for token in ("изоляция из вспененного каучука трубная", "изоляция из вспененного полиэтилена трубная")) or (
            any(token in search_text for token in ("изоляция из вспененного каучука трубная", "изоляция из вспененного полиэтилена трубная", "трубная изоляция", "вспененного каучука", "вспененного полиэтилена", "pipe insulation", "rubber pipe insulation", "polyethylene pipe insulation"))
        ...
        if (
            ...
            return "transformer"
        if "устройства компенсации реактивной мощности 0 4кв" in branch_path or (
            any(token in search_text for token in ("укрм", "компенсация реактивной мощности", "реактивной мощности", "capacitor bank", "power factor correction"))
            and not any(token in search_text for token in ("стабилизатор", "трансформатор", "ибп", "ups"))
        ):
        ...
        if any(token in branch_path for token in ("преобразователи частоты приводы", "аксессуары для преобразователей частоты", "сервисные позиции запасные части для преобразователей частоты")) or (
            any(
                token in search_text
                for token in (
        ...
        if any(
            ...
            (
                ...
                or (
                    any(token in search_text for token in ("щмп", "щит", "щитка", "щитовые", "корпус"))
                    and any(token in search_text for token in ("козырек", "кожух", "петл", "замок", "окно", "крепеж", "переходник", "планка", "уголок"))
                )
            )
            and not any(token in search_text for token in ("печатная плата", "pcb", "rj45", "keystone", "патч"))
        ):
        ...
        if any(
            ...
            any(token in search_text for token in ("щит распредел", "силовой щит", "щит с монтажной панелью", "щит этажный", "щиток", "электрощит", "корпус распредел", "корпус учетно", "корпус для модульных устройств", "корпус мультимедиа", "бокс", "щрв", "щрн", "щурв", "щурн", "щмп"))
            and any(token in search_text for token in ("встраив", "навес", "модул", "распредел", "учет", "щит", "этажн", "монтажн", "мультимедиа"))
            and not any(token in search_text for token in ("заглуш", "рамк", "аксессуар", "комплектующ", "козырек", "кожух", "петл", "замок", "окно герметичное", "комплект для навески", "щитовые аксессуары"))
        ):
        ...
        if "перемычки для клемм на din рейку" in branch_path or (
            any(token in search_text for token in ("перемычки для клемм на din рейку", "перемычка для клемм на din рейку", "terminal block accessory", "terminal block jumper", "din rail terminal accessory"))
            and not any(token in search_text for token in ("клеммный блок", "клеммник для печатной платы", "аккумулятор", "акб"))
        ):
        ...
        if "штыревые втулочные наконечники" in branch_path or (
            ...
            and not any(token in search_text for token in ("клеммный блок", "клеммник", "din рейк", "din-рейк", "terminal block", "клещ", "клещи", "пресс", "обжим", "кримпер"))
        ):
            return "wire_ferrule"
        ...
        if "изолента" in branch_path or (
            any(token in search_text for token in ("изолента", "изоляционная лента", "лента изоляционная", "electrical tape", "insulation tape", "pvc tape"))
            and not any(token in search_text for token in ("малярная лента", "скотч", "denso", "упаковочная лента"))
        ):
            return "electrical_tape"
        if "ip видеокамеры" in branch_path or (
            any(token in search_text for token in ("ip-видеокамера", "ip видеокамера", "ip-видеокамеры", "ip camera", "network camera"))
            and not any(token in search_text for token in ("аналог", "hdcvi", "ahd", "tv line", "видеорегистратор"))
        ):
            return "ip_camera"
        if (
