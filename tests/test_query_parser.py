...
class QueryParserTests(unittest.TestCase):
    ...
    def test_parse_query_spec_detects_hacksaws_bits_pliers_and_din_rail_queries(self):
        hacksaw = parse_query_spec("Ножовка по металлу 300 мм", taxonomy_rules=self.rules)
        bit_set = parse_query_spec("Набор бит 32 предмета", taxonomy_rules=self.rules)
        pozidriv = parse_query_spec("Бита PZ2 50 мм", taxonomy_rules=self.rules)
        sandpaper = parse_query_spec("Бумага шлифовальная P120", taxonomy_rules=self.rules)
        pliers = parse_query_spec("Пассатижи 180 мм", taxonomy_rules=self.rules)
        groove_plier = parse_query_spec("Клещи переставные 250 мм", taxonomy_rules=self.rules)
        flange = parse_query_spec("Фланец стальной плоский DN50", taxonomy_rules=self.rules)
        electrode = parse_query_spec("Электрод сварочный УОНИ 13/55", taxonomy_rules=self.rules)
        din_rail = parse_query_spec("DIN-рейка оцинкованная 35мм", taxonomy_rules=self.rules)
        metal_hole_saw = parse_query_spec("Коронка по металлу биметаллическая 35 мм", taxonomy_rules=self.rules)
        summer_jacket = parse_query_spec("Куртка летняя рабочая", taxonomy_rules=self.rules)
        dimmer = parse_query_spec("Диммер скрытого монтажа 600Вт", taxonomy_rules=self.rules)

        self.assertEqual(hacksaw.entity_type, "hacksaw")
        self.assertEqual(hacksaw.branch_hint, "ножовки")
        self.assertEqual(bit_set.entity_type, "driver_bit_set")
        self.assertEqual(bit_set.branch_hint, "наборы бит")
        self.assertEqual(pozidriv.entity_type, "pozidriv_bit")
        self.assertEqual(pozidriv.branch_hint, "биты крест PZ (Pozidriv)")
        self.assertEqual(sandpaper.entity_type, "sandpaper")
        self.assertEqual(sandpaper.branch_hint, "бумага шлифовальная")
        self.assertEqual(pliers.entity_type, "combination_pliers")
        self.assertEqual(pliers.branch_hint, "плоскогубцы и пассатижи")
        self.assertEqual(groove_plier.entity_type, "tongue_groove_plier")
        self.assertEqual(groove_plier.branch_hint, "переставные клещи")
        self.assertEqual(flange.entity_type, "steel_flange")
        self.assertEqual(flange.branch_hint, "фланцы стальные плоские")
        self.assertEqual(electrode.entity_type, "welding_electrode")
        self.assertEqual(electrode.branch_hint, "электроды для сварки")
        self.assertEqual(din_rail.entity_type, "din_rail")
        self.assertEqual(din_rail.branch_hint, "din-рейки")
        self.assertEqual(metal_hole_saw.entity_type, "hole_saw")
        self.assertEqual(metal_hole_saw.branch_hint, "коронки по металлу")
        self.assertEqual(summer_jacket.entity_type, "workwear")
        self.assertEqual(summer_jacket.branch_hint, "куртки летние")
        self.assertEqual(dimmer.entity_type, "switch_wiring")
        self.assertEqual(dimmer.branch_hint, "светорегуляторы (диммеры) скрытого монтажа")
