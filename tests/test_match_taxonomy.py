...
class MatchTaxonomyTests(unittest.TestCase):
    ...
    def test_effective_candidate_family_maps_other_voltage_indicator_branch(self):
        ...
        candidate = {
            "name": "Индикатор напряжения двухполюсный 12-690В",
            "normalized_name": "индикатор напряжения двухполюсный 12 690в",
            "branch_path": "индикаторы напряжения",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_voltage_stabilizer_branch(self):
        features = self.matcher._extract_query_features("Стабилизатор напряжения 10 кВА 220В")
        features["entity_type"] = "voltage_stabilizer"
        candidate = {
            "name": "Стабилизатор напряжения 10 кВА 220В",
            "normalized_name": "стабилизатор напряжения 10 ква 220в",
            "branch_path": "стабилизаторы напряжения",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_electric_motor_branch(self):
        features = self.matcher._extract_query_features("Электродвигатель асинхронный трехфазный 5,5 кВт")
        features["entity_type"] = "electric_motor"
        candidate = {
            "name": "Электродвигатель асинхронный трехфазный 5,5 кВт",
            "normalized_name": "электродвигатель асинхронный трехфазный 5 5 квт",
            "branch_path": "электродвигатели общепромышленные",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_industrial_pump_branch(self):
        features = self.matcher._extract_query_features("Насос промышленный вертикальный центробежный 5,5 кВт")
        features["entity_type"] = "industrial_pump"
        candidate = {
            "name": "Насос промышленный вертикальный центробежный 5,5 кВт",
            "normalized_name": "насос промышленный вертикальный центробежный 5 5 квт",
            "branch_path": "промышленные вертикальные центробежные насосы",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_drill_bit_metal_branch(self):
        ...
        candidate = {
            "name": "Сверло по металлу HSS 8 мм",
            "normalized_name": "сверло по металлу hss 8 мм",
            "branch_path": "сверла по металлу",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_masonry_drill_bit_branch(self):
        features = self.matcher._extract_query_features("Бур SDS-Plus 8x160 мм")
        features["entity_type"] = "masonry_drill_bit"
        candidate = {
            "name": "Бур SDS-Plus 8x160 мм",
            "normalized_name": "бур sds plus 8x160 мм",
            "branch_path": "буры sds-plus",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_sds_chisel_branch(self):
        features = self.matcher._extract_query_features("Зубило SDS-Plus плоское 20x250 мм")
        features["entity_type"] = "sds_chisel"
        candidate = {
            "name": "Зубило SDS-Plus плоское 20x250 мм",
            "normalized_name": "зубило sds plus плоское 20x250 мм",
            "branch_path": "зубила sds-plus",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_effective_candidate_family_maps_other_wire_ferrule_branch(self):
        features = self.matcher._extract_query_features("Наконечник штыревой втулочный НШВИ 1,5-8")
        features["entity_type"] = "wire_ferrule"
        candidate = {
            "name": "Наконечник штыревой втулочный НШВИ 1,5-8",
            "normalized_name": "наконечник штыревой втулочный ншви 1 5 8",
            "branch_path": "штыревые втулочные наконечники (ншв и ншви)",
            "entity_type": "other",
            "item_markers": {},
        }
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_wire_ferrule_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_voltage_stabilizer_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_voltage_indicator_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
        ...
        self.matcher._collect_branch_candidates(
            ...
            limit=25,
            query_features={"entity_type": "voltage_indicator"},
        )
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_electric_motor_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
        ...
        self.matcher._collect_branch_candidates(
            ...
            limit=25,
            query_features={"entity_type": "electric_motor"},
        )

        self.assertNotIn("search_entity_type", captured["where_sql"])
        self.assertEqual(captured["limit"], 25)

    def test_collect_branch_candidates_relaxes_entity_filter_for_industrial_pump_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_drill_bit_metal_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
        ...
        self.matcher._collect_branch_candidates(
            ["сверла по металлу"],
            limit=25,
            query_features={"entity_type": "drill_bit_metal"},
        )

        self.assertNotIn("search_entity_type", captured["where_sql"])
        self.assertEqual(captured["limit"], 25)

    def test_collect_branch_candidates_relaxes_entity_filter_for_masonry_drill_bit_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
    ...
    def test_collect_branch_candidates_relaxes_entity_filter_for_sds_chisel_family(self):
        self.matcher._uses_duckdb_query_backend = lambda: True
        captured = {}
        ...
        self.matcher._collect_branch_candidates(
            ["зубила sds-plus"],
            limit=25,
            query_features={"entity_type": "sds_chisel"},
        )
