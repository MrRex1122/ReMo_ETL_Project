...
def classify_item_type(
    ...
    if (
        any(
            ...
            for token in (
                ...
                "частотный привод",
                "аксессуар для преобразователя частоты",
                "аксессуары для преобразователей частоты",
                "запасные части для преобразователя частоты",
                "сервисные позиции для преобразователя частоты",
                "frequency drive",
    ...
    if (
        ...
        or ("клапан" in normalized and any(token in normalized for token in ("регулирующ", "запорн", "вентил", "обратн")))
        or "задвижк" in normalized
    ):
    ...
    if (
        any(token in normalized for token in ("метчик", "метчики", "tap", "thread tap"))
    ...
    if (
        ...
        return "thread_gauge"
    if (
        any(token in normalized for token in ("набор резьбонарезного инструмента", "наборы резьбонарезного инструмента", "tap and die set", "threading tool set"))
        and not any(token in normalized for token in ("резьбомер", "калибр", "сверл", "коронк", "фрез"))
    ):
        return "threading_tool_set"
    if (
    ...
    if (
        any(token in normalized for token in ("леска для триммеров", "леска для триммера", "триммерная леска", "trimmer line", "nylon line"))
        and not any(token in normalized for token in ("кабель", "провод", "wire", "fishing", "рыболов"))
    ):
        return "trimmer_line"
    if (
    ...
    if (
        (
            any(token in normalized for token in ("фрезы и наборы фрез для ручных фрезеров", "фреза для ручного фрезера", "набор фрез для ручного фрезера", "router bit", "router cutter"))
            or ("фрез" in normalized and "фрезер" in normalized and any(token in normalized for token in ("ручн", "router")))
        )
        and not any(token in normalized for token in ("фрезы для станков", "milling cutter", "коронка", "сверло", "диск"))
    ):
        return "router_bit"
    if (
        any(
            ...
            or ("костюм" in normalized and "сварщ" in normalized)
        )
    ...
    if (
        any(token in normalized for token in ("защитные очки", "очки защитные", "safety glasses", "protective eyewear"))
        and not any(token in normalized for token in ("очки виртуальной реальности", "солнцезащитные", "маска сварщика", "щиток лицевой"))
    ):
        return "safety_glasses"
    if (
        any(token in normalized for token in ("каска", "каски", "защитная каска", "safety helmet", "hard hat"))
        and not any(token in normalized for token in ("мотошлем", "велошлем", "шлем сварщика", "маска сварщика"))
    ):
        return "protective_helmet"
    if (
    ...
    if (
        any(token in normalized for token in ("накидной ключ", "накидные ключи", "ключ накидной", "ring wrench", "box wrench", "box-end wrench"))
        and not any(token in normalized for token in ("комбинирован", "рожков", "имбус", "шестигран", "разводной", "трубный ключ", "ключ доступа"))
    ):
        return "ring_wrench"
    if (
        any(token in normalized for token in ("разводной ключ", "разводные ключи", "ключ разводной", "adjustable wrench", "crescent wrench"))
        and not any(token in normalized for token in ("комбинирован", "рожков", "накидн", "имбус", "шестигран", "трубный ключ", "ключ доступа"))
    ):
        return "adjustable_wrench"
    if (
    ...
    if (
        ...
        return "micrometer"
    if (
        any(token in normalized for token in ("нутромер", "нутромеры", "bore gauge", "inside gauge"))
        and not any(token in normalized for token in ("микрометр", "штангенциркуль", "резьбомер", "индикатор часового типа"))
    ):
        return "bore_gauge"
    if (
    ...
    if (
        ...
        return "paint_brush"
    if (
        any(token in normalized for token in ("корщетка", "корщетки", "крацовка", "щетка крацовка", "wire brush wheel", "cup brush"))
    ...
    if (
        any(token in normalized for token in ("хомуты для труб", "хомут для труб", "трубный хомут", "pipe clamp", "pipe hanger"))
        and not any(token in normalized for token in ("кабельная стяжка", "хомут червячный", "хомут силовой", "strut clamp"))
    ):
        return "pipe_clamp"
    if (
        (
            any(token in normalized for token in ("фитинги для наружной канализации", "фитинги для внутренней канализации", "канализационный фитинг", "sewer fitting", "drain fitting"))
            or (
                any(token in normalized for token in ("отвод", "тройник", "муфта", "ревизи", "крестовин", "переход"))
                and any(token in normalized for token in ("канализ", "наружн", "внутрен", "sewer", "drain"))
            )
            or "фитинги шумопоглощающие для канализации" in normalized
            or ("шумопоглоща" in normalized and "канализ" in normalized)
        )
        and not any(token in normalized for token in ("pex", "pert", "пнд", "полипропилен", "латун", "трубный хомут", "сифон"))
    ):
        return "sewer_fitting"
    if (
        any(token in normalized for token in ("сифон", "сифоны", "siphon", "bottle trap", "trap for sink"))
        and not any(token in normalized for token in ("фитинг", "канализационная труба", "трап", "дренажный клапан"))
    ):
        return "siphon"
    if (
    ...
    if (
        any(token in normalized for token in ("специальный инструмент для авторемонта", "инструмент для авторемонта", "вакуумметр", "ареометр", "маслосборный", "противооткатный башмак", "авторемонтный инструмент"))
    ...
    if (
        any(token in normalized for token in ("набор отверток", "наборы отверток", "screwdriver set", "set of screwdrivers"))
        and not any(token in normalized for token in ("битодержатель", "набор бит", "torx bit", "phillips bit"))
    ):
        return "screwdriver_set"
    if (
        any(token in normalized for token in ("бокорез", "бокорезы", "кусачки", "диагональные кусачки", "side cutter", "diagonal cutter", "nipper"))
    ...
    if (
        ...
        return "clamp_tool"
    if (
        any(token in normalized for token in ("съемник", "съемники ручные", "puller tool", "gear puller", "bearing puller"))
        and not any(token in normalized for token in ("экстрактор", "домкрат", "подъемник", "магнитный съемник"))
    ):
        return "manual_puller"
    if (
        any(token in normalized for token in ("домкрат", "домкраты", "hydraulic jack", "bottle jack", "floor jack"))
        and not any(token in normalized for token in ("стойка", "тележка", "подкатная тележка", "опора"))
    ):
        return "jack"
    if (
    ...
    if (
        any(token in normalized for token in ("сверло по дереву", "сверла по дереву", "wood drill", "brad point", "auger bit"))
        and not any(token in normalized for token in ("металл", "metal", "бетон", "concrete", "коронк", "бур", "sds", "метчик", "tap"))
    ):
        return "wood_drill_bit"
    if (
        any(
            token in normalized
    ...
    if (
        any(token in normalized for token in ("коронка по бетону", "коронки по бетону", "алмазная коронка по бетону", "diamond hole saw", "core bit"))
    ...
    if "радиатор" in normalized or "radiator" in normalized:
        return "radiator"
    if ("конвектор" in normalized or "convector" in normalized) and any(token in normalized for token in ("внутрипол", "напольн", "floor")):
        return "floor_convector"
    if (
        (
            any(token in normalized for token in ("конвекторы электрические", "конвектор электрический", "электрический конвектор", "электроконвектор", "electric convector", "electric heater convector"))
            or (("конвектор" in normalized or "convector" in normalized) and any(token in normalized for token in ("электр", "electric")))
        )
        and not any(token in normalized for token in ("внутрипольный", "напольный", "floor convector", "радиатор"))
    ):
        return "electric_convector"
    if (
        (
            any(
                token in normalized
            ...
            or ("водонагреватель" in normalized and "накоп" in normalized)
        )
        and not any(token in normalized for token in ("проточный", "газовый", "теплообменник"))
    ):
        return "storage_water_heater"
    if (
        any(token in normalized for token in ("нагревательный мат", "нагревательные маты", "тёплый пол мат", "теплый пол мат", "heating mat"))
        and not any(token in normalized for token in ("кабель нагревательный", "терморегулятор", "тепловентилятор", "коврик"))
    ):
        return "heating_mat"
    ...
    ) and not any(token in normalized for token in ("кабель", "электро", "лента")):
    ...
    if (
        any(token in normalized for token in ("смеситель", "смесители для ванны и душа", "смесители для мойки", "faucet", "mixer tap"))
        and not any(token in normalized for token in ("картридж", "ремкомплект", "душевая стойка", "лейка"))
    ):
        return "faucet"
    if (
    ...
    if (
        any(token in normalized for token in ("устройство компенсации реактивной мощности", "компенсация реактивной мощности", "конденсаторная установка", "конденсаторные установки", "укрм", "reactive power compensation", "capacitor bank"))
        and not any(token in normalized for token in ("конденсатор пусковой", "конденсатор моторный"))
    ):
        return "reactive_power_compensator"
    if any(token in normalized for token in ("изолента", "изоляционная лента", "лента изоляционная", "electrical tape", "insulating tape")) and not any(
        token in normalized for token in ("теплоизоляц", "вспененн", "фум", "скотч", "малярная лента")
    ):
    ...
    if any(
        ...
        return "wire_ferrule"
    if (
        any(token in normalized for token in ("силовые медные луженые наконечники", "наконечники тмл", "наконечник тмл", "луженый кабельный наконечник", "cable lug", "terminal lug"))
        and not any(token in normalized for token in ("ншв", "ншви", "втулоч", "клеммный блок", "клеммник", "клещи", "обжим"))
    ):
        return "power_terminal_lug"
    if any(
        token in normalized
        for token in (
            "клеммный блок",
    ...
    if (
        any(token in normalized for token in ("перемычки для клемм на din-рейку", "маркеры для клемм на din-рейку", "перемычка для клемм", "маркер для клемм", "terminal block jumper", "terminal block marker"))
        and not any(token in normalized for token in ("аккумулятор", "акб"))
    ):
        return "terminal_block_accessory"
    if (
        any(token in normalized for token in ("ip-видеокамеры", "ip видеокамера", "ip camera", "network camera", "видеокамера ip"))
        and not any(token in normalized for token in ("муляж", "кронштейн", "объектив", "регистратор"))
    ):
        return "ip_camera"
    if any(
        token in normalized
...
def _normalize_effective_entity_type_by_catalog_branch(
    *,
    ...
    if any(
        ...
        return "switchboard_accessory"
    if any(marker in normalized_branch for marker in ("устройства компенсации реактивной мощности 0.4кв", "конденсаторные установки")):
        return "reactive_power_compensator"
    ...
    if any(marker in normalized_branch for marker in ("металлорукав с изоляцией", "гофрированные трубы для прокладки кабеля", "трубы жесткие двустенные")):
        return "cable_conduit"
    if any(marker in normalized_branch for marker in ("затворы поворотные дисковые", "краны шаровые стальные", "краны шаровые латунные для воды", "краны шаровые пнд", "клапаны электромагнитные соленоидные", "клапаны регулирующие чугунные", "клапаны запорные вентили стальные", "клапаны обратные чугунные", "задвижки чугунные клиновые")):
        return "industrial_valve"
    ...
    if "резьбомеры" in normalized_branch:
        return "thread_gauge"
    if "наборы резьбонарезного инструмента" in normalized_branch:
        return "threading_tool_set"
    ...
    if "фрезы и наборы фрез для ручных фрезеров" in normalized_branch:
        return "router_bit"
    if any(marker in normalized_branch for marker in ("костюмы летние", "костюмы утепленные", "костюмы сварщика", "брюки полукомбинезоны", "куртки утепленные")):
        return "workwear"
    ...
    if "антипорезные и защитные перчатки" in normalized_branch:
        return "protective_gloves"
    if "защитные очки" in normalized_branch:
        return "safety_glasses"
    if "каски" in normalized_branch:
        return "protective_helmet"
    ...
    if "рожковые ключи" in normalized_branch:
        return "open_end_wrench"
    if "накидные ключи" in normalized_branch:
        return "ring_wrench"
    if "разводные ключи" in normalized_branch:
        return "adjustable_wrench"
    ...
    if "микрометры" in normalized_branch:
        return "micrometer"
    if "нутромеры" in normalized_branch:
        return "bore_gauge"
    ...
    if "шлицевые отвертки" in normalized_branch:
        return "slotted_screwdriver"
    if "наборы отверток" in normalized_branch:
        return "screwdriver_set"
    ...
    if "струбцины" in normalized_branch:
        return "clamp_tool"
    if "съемники ручные" in normalized_branch:
        return "manual_puller"
    if "домкраты" in normalized_branch:
        return "jack"
    ...
    if "соединители для труб" in normalized_branch:
        return "pipe_connector"
    if "хомуты для труб" in normalized_branch:
        return "pipe_clamp"
    if any(marker in normalized_branch for marker in ("фитинги для наружной канализации", "фитинги для внутренней канализации", "фитинги шумопоглощающие для канализации")):
        return "sewer_fitting"
    if "сифоны" in normalized_branch:
        return "siphon"
    ...
    if "сверла по металлу" in normalized_branch:
        return "drill_bit_metal"
    if "сверла по дереву" in normalized_branch:
        return "wood_drill_bit"
    ...
    if "радиаторы стальные панельные" in normalized_branch:
        return "radiator"
    if any(marker in normalized_branch for marker in ("конвекторы внутрипольные", "конвекторы напольные")):
        return "floor_convector"
    if "конвекторы электрические" in normalized_branch:
        return "electric_convector"
    if "нагревательные маты" in normalized_branch:
        return "heating_mat"
    if "коллекторные группы для теплого пола" in normalized_branch:
        return "floor_heating_manifold"
    if "водонагреватели электрические накопительные" in normalized_branch:
        return "storage_water_heater"
    if "термоусаживаемые изделия" in normalized_branch:
        return "heat_shrink"
    if any(marker in normalized_branch for marker in ("изоляция из вспененного каучука трубная", "изоляция из вспененного полиэтилена трубная")):
        return "pipe_insulation"
    if any(
        marker in normalized_branch
    ...
    if "регулятор давления" in normalized_branch:
        return "pressure_regulator"
    if any(marker in normalized_branch for marker in ("смесители для ванны и душа", "смесители для мойки")):
        return "faucet"
    if "стабилизаторы напряжения" in normalized_branch:
        return "voltage_stabilizer"
    if "электродвигатели общепромышленные" in normalized_branch:
        return "electric_motor"
    if any(marker in normalized_branch for marker in ("преобразователи частоты приводы", "аксессуары для преобразователей частоты", "сервисные позиции запасные части для преобразователей частоты")):
        return "frequency_drive"
    if "удлинители сетевые фильтры переходники штепсельные вилки" in normalized_branch:
        return "power_accessory"
    if "ip видеокамеры" in normalized_branch:
        return "ip_camera"
    if any(
        marker in normalized_branch
        for marker in (
    ...
    if "штыревые втулочные наконечники" in normalized_branch:
        return "wire_ferrule"
    if "силовые медные луженые наконечники тмл" in normalized_branch:
        return "power_terminal_lug"
    if any(marker in normalized_branch for marker in ("перемычки для клемм на din рейку", "маркеры для клемм на din рейку")):
        return "terminal_block_accessory"
    if "светосигнальная арматура" in normalized_branch:
        return "signal_indicator"
    if "изолента" in normalized_branch:
        return "electrical_tape"
    if any(marker in normalized_branch for marker in ("ножи строительные", "лезвия для ножей")):
        return "utility_knife"
    ...
    *texts: str,
    ...
    if catalog_row_mode:
        ...
        if registry_defaults and registry_defaults[0] != "прочее":
            ...
            if effective_family == "industrial_valve":
                ...
                if "регулирующ" in merged and "чугун" in merged:
                ...
                if "запорн" in merged or "вентил" in merged:
                    return "клапаны запорные (вентили) стальные"
                if "пнд" in merged:
            ...
            if effective_family == "thread_gauge":
                return "резьбомеры"
            if effective_family == "threading_tool_set":
            ...
            if effective_family == "router_bit":
                return "фрезы и наборы фрез для ручных фрезеров"
            ...
            if effective_family == "safety_glasses":
            ...
            if effective_family == "ring_wrench":
                return "накидные ключи"
            if effective_family == "adjustable_wrench":
                return "разводные ключи"
            if effective_family == "screwdriver_set":
                return "наборы отверток"
            if effective_family == "hex_bit":
                return "биты шестигранные HEX"
            ...
            if effective_family == "micrometer":
                return "микрометры"
            if effective_family == "bore_gauge":
                return "нутромеры"
            ...
            if effective_family == "wire_brush_tool":
                return "корщетки"
            if effective_family == "hook_loop_sanding_disc":
            ...
            if effective_family == "clamp_tool":
                return "струбцины"
            if effective_family == "manual_puller":
                return "съемники ручные"
            if effective_family == "torx_bit":
                return "биты TORX"
            if effective_family == "phillips_bit":
                return "биты крест PH (Phillips)"
            if effective_family == "pipe_clamp":
                return "хомуты для труб"
            if effective_family == "sewer_fitting":
                if "шумопоглоща" in merged:
                    return "фитинги шумопоглощающие для канализации"
                if "наружн" in merged:
                    return "фитинги для наружной канализации"
                return "фитинги для внутренней канализации"
            if effective_family == "siphon":
                return "сифоны"
            if effective_family == "self_tapping_screw":
                if "гипсокартон" in merged:
                    if "металл" in merged:
                        return "саморезы гипсокартон-металл"
                    return "саморезы гипсокартон-дерево"
                return "саморезы универсальные"
            if effective_family == "drill_bit_metal":
                return "сверла по металлу"
            if effective_family == "wood_drill_bit":
                return "сверла по дереву"
            ...
            if effective_family == "countersink_tool":
            ...
            if effective_family == "floor_convector":
                if "напольн" in merged:
                    return "конвекторы напольные"
                return "конвекторы внутрипольные"
            if effective_family == "electric_convector":
                return "конвекторы электрические"
            if effective_family == "floor_heating_manifold":
                return "коллекторные группы для теплого пола"
            if effective_family == "storage_water_heater":
                return "водонагреватели электрические накопительные"
            if effective_family == "heating_mat":
                return "нагревательные маты"
            ...
            if effective_family == "faucet":
                if "мойк" in merged:
                    return "смесители для мойки"
                return "смесители для ванны и душа"
            ...
            if effective_family == "frequency_drive":
                if any(token in merged for token in ("сервис", "запасн", "запчаст", "ремкомплект")):
                    return "сервисные позиции (запасные части) для преобразователей частоты"
                if "аксессуар" in merged:
                    return "аксессуары для преобразователей частоты"
            ...
            if effective_family == "reactive_power_compensator":
                if "конденсаторн" in merged:
                    return "конденсаторные установки"
                return "устройства компенсации реактивной мощности 0.4кв"
            if effective_family == "distribution_enclosure":
                ...
                if "щмп" in merged and "щит" in merged:
                    return "корпуса щитов монтажных металлических"
            ...
            if effective_family == "push_button":
                ...
                return "кнопки"
            if effective_family == "terminal_block":
                if "печатн" in merged and ("плат" in merged or "pcb" in merged):
            ...
            if effective_family == "terminal_block_accessory":
                if "маркер" in merged:
                    return "маркеры для клемм на din-рейку"
                return "перемычки для клемм на din-рейку"
            if effective_family == "power_terminal_lug":
                return "силовые медные луженые наконечники (тмл)"
            if effective_family == "electrical_tape":
                return "изолента"
            if effective_family == "ip_camera":
                return "ip-видеокамеры"
            if effective_family == "jack":
                return "домкраты"
            if effective_family == "utility_knife":
                if "лезви" in merged:
                    return "лезвия для ножей"
                return "ножи строительные"
            if effective_family == "trimmer_line":
    ...
    if registry_defaults and registry_defaults[0] != "прочее":
        ...
        if registry_family == "thread_gauge":
            return "резьбомеры"
        if registry_family == "threading_tool_set":
            return "наборы резьбонарезного инструмента"
        ...
        if registry_family == "protective_gloves":
            return "антипорезные и защитные перчатки"
        if registry_family == "safety_glasses":
            return "защитные очки"
        ...
        if registry_family == "open_end_wrench":
            return "рожковые ключи"
        if registry_family == "ring_wrench":
            return "накидные ключи"
        if registry_family == "adjustable_wrench":
            return "разводные ключи"
        if registry_family == "screwdriver_set":
            return "наборы отверток"
        if registry_family == "hex_bit":
            return "биты шестигранные HEX"
        ...
        if registry_family == "drill_bit_metal":
            return "сверла по металлу"
        if registry_family == "wood_drill_bit":
            return "сверла по дереву"
        ...
        if registry_family == "floor_convector":
            if "напольн" in merged:
                return "конвекторы напольные"
            return "конвекторы внутрипольные"
        if registry_family == "electric_convector":
            return "конвекторы электрические"
        if registry_family == "floor_heating_manifold":
            return "коллекторные группы для теплого пола"
        if registry_family == "storage_water_heater":
            return "водонагреватели электрические накопительные"
        if registry_family == "heating_mat":
            return "нагревательные маты"
        if registry_family == "heat_shrink":
            return "термоусаживаемые изделия"
        if registry_family == "pipe_insulation":
            if "полиэтилен" in merged:
                return "изоляция из вспененного полиэтилена трубная"
        ...
        if registry_family == "pressure_regulator":
            return "регулятор давления"
        if registry_family == "faucet":
            if "мойк" in merged:
                return "смесители для мойки"
            return "смесители для ванны и душа"
        if registry_family == "voltage_stabilizer":
            return "стабилизаторы напряжения"
        if registry_family == "reactive_power_compensator":
            if "конденсаторн" in merged:
                return "конденсаторные установки"
            return "устройства компенсации реактивной мощности 0.4кв"
        ...
        if registry_family == "frequency_drive":
            if any(token in merged for token in ("сервис", "запасн", "запчаст")):
                return "сервисные позиции (запасные части) для преобразователей частоты"
            if "аксессуар" in merged:
                return "аксессуары для преобразователей частоты"
            return "преобразователи частоты, приводы"
        ...
        if registry_family == "power_terminal_lug":
            return "силовые медные луженые наконечники (тмл)"
        if registry_family == "signal_indicator":
            return "светосигнальная арматура"
        if registry_family == "electrical_tape":
            return "изолента"
        if registry_family == "ip_camera":
            return "ip-видеокамеры"
        if registry_family == "utility_knife":
            if "лезви" in merged:
                return "лезвия для ножей"
            return "ножи строительные"
    ...
    if effective_entity_type == "voltage_indicator":
        return "индикаторы напряжения"
    if effective_entity_type == "reactive_power_compensator":
        if "конденсаторн" in merged:
            return "конденсаторные установки"
        return "устройства компенсации реактивной мощности 0.4кв"
    if effective_entity_type == "faucet":
        if "мойк" in merged:
            return "смесители для мойки"
        return "смесители для ванны и душа"
    ...
    if effective_entity_type == "frequency_drive":
        if any(token in merged for token in ("запасн", "сервисн", "ремкомплект", "spare", "service")):
            return "сервисные позиции (запасные части) для преобразователей частоты"
        if "аксессуар" in merged:
            return "аксессуары для преобразователей частоты"
        return "преобразователи частоты, приводы"
    ...
    if effective_entity_type == "industrial_pump":
        return "промышленные вертикальные центробежные насосы"
    if effective_entity_type == "industrial_valve":
        if "регулирующ" in merged and "чугун" in merged:
            return "клапаны регулирующие чугунные"
        if "обратн" in merged and "чугун" in merged:
            return "клапаны обратные чугунные"
        ...
        if "кран" in merged and "шар" in merged:
            return "краны шаровые стальные"
        if any(token in merged for token in ("соленоид", "электромагнит")):
            return "клапаны электромагнитные (соленоидные)"
        if "чугун" in merged:
            return "затворы поворотные дисковые чугунные"
        return "затворы поворотные дисковые стальные"
    ...
    if effective_entity_type == "thread_gauge":
        return "резьбомеры"
    if effective_entity_type == "threading_tool_set":
        return "наборы резьбонарезного инструмента"
    ...
    if effective_entity_type == "milling_cutter":
        return "фрезы для станков"
    if effective_entity_type == "router_bit":
        return "фрезы и наборы фрез для ручных фрезеров"
    if effective_entity_type == "workwear":
        if "костюмы сварщика" in normalized_branch:
            return "костюмы сварщика"
        if "сварщ" in merged:
            return "костюмы сварщика"
        if "куртк" in merged and "утеплен" in merged:
    ...
    if effective_entity_type == "protective_gloves":
        return "антипорезные и защитные перчатки"
    if effective_entity_type == "safety_glasses":
        return "защитные очки"
    if effective_entity_type == "protective_helmet":
        return "каски"
    ...
    if effective_entity_type == "open_end_wrench":
        return "рожковые ключи"
    if effective_entity_type == "ring_wrench":
        return "накидные ключи"
    if effective_entity_type == "adjustable_wrench":
        return "разводные ключи"
    if effective_entity_type == "screwdriver_set":
        return "наборы отверток"
    if effective_entity_type == "hex_bit":
        return "биты шестигранные HEX"
    ...
    if effective_entity_type == "micrometer":
        return "микрометры"
    if effective_entity_type == "bore_gauge":
        return "нутромеры"
    ...
    if effective_entity_type == "sewer_fitting":
        ...
        if "наружн" in merged:
            return "фитинги для наружной канализации"
        return "фитинги для внутренней канализации"
    if effective_entity_type == "siphon":
        return "сифоны"
    ...
    if effective_entity_type == "drill_bit_metal":
        return "сверла по металлу"
    if effective_entity_type == "wood_drill_bit":
        return "сверла по дереву"
    ...
    if effective_entity_type == "sds_chisel":
        if "sds-max" in merged or "sds max" in merged:
            return "зубила sds-max"
        return "зубила sds-plus"
    if effective_entity_type == "floor_convector":
        if "напольн" in merged:
            return "конвекторы напольные"
        return "конвекторы внутрипольные"
    if effective_entity_type == "electric_convector":
        return "конвекторы электрические"
    if effective_entity_type == "floor_heating_manifold":
        return "коллекторные группы для теплого пола"
    if effective_entity_type == "storage_water_heater":
        return "водонагреватели электрические накопительные"
    if effective_entity_type == "heating_mat":
        return "нагревательные маты"
    ...
    if effective_entity_type == "wire_ferrule":
        return "штыревые втулочные наконечники (ншв и ншви)"
    if effective_entity_type == "power_terminal_lug":
        return "силовые медные луженые наконечники (тмл)"
    if effective_entity_type == "signal_indicator":
        return "светосигнальная арматура"
    if effective_entity_type == "electrical_tape":
        return "изолента"
    if effective_entity_type == "ip_camera":
        return "ip-видеокамеры"
    if effective_entity_type == "utility_knife":
        if "лезви" in merged:
            return "лезвия для ножей"
        return "ножи строительные"
    if effective_entity_type == "trimmer_line":
        return "леска для триммеров"
